...
- **Batalla**: partido 1c1 **en tiempo real** (estilo FIFA) sobre el mini-campo (canvas `#pitch`, 960×420), al mejor de 3 goles y **a pantalla completa** (`showView` oculta cabecera y nav en `battle-view`; todo el HUD del partido son elementos flotantes sobre `#battlefield`, y el resultado/fichaje vive en el panel `#match-result`). Objeto global `battle` (o `null`). Flujo: `startEncounter()`/`startBossBattle()` → `beginBattle()` → `setupBattle()` → `startPlay()`; `updateMatch(dt)` corre desde el bucle principal y mueve jugador (entrada en `battleMove()`: joystick táctil flotante `joy` con prioridad sobre `battleKeys` de teclado), IA rival (`updateFoe`), balón (`updateBall` + `handleGoalLines`) y porteros; `goalFor()` → `winMatch()`/`loseMatch()`; tras ganar, `contract()` para fichar; `endBattle()` cierra. Guarda cada callback diferido con `if (!battle) return` porque el jugador puede abandonar.
