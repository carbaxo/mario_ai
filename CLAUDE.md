...
- **Batalla**: partido 1c1 al mejor de 3 goles sobre un mini-campo animado (canvas `#pitch`, 960×420). Objeto global `battle` (o `null`). Flujo: `startEncounter()`/`startBossBattle()` → `beginBattle()` → `setupBattle()` → rondas `attack()` → `rivalPlay()` hasta `winMatch()`/`loseMatch()`; tras ganar, `contract()` para fichar; `endBattle()` cierra. El candado de ronda es `battle.busy`; guarda cada callback diferido con `if (!battle) return` porque el jugador puede abandonar. La escena (carrera, disparo, gol/parada, confeti) vive en `battle.scene` y la dibuja `drawPitch()` desde el bucle principal; las animaciones son cosméticas — el resultado se sortea al pulsar el botón.
- **Matemática del duelo**: `goalChance(tipo)` (tu % de gol, visible en vivo en cada botón) y `rivalChance(tuÚltimaJugada)` (el contraataque: el disparo te expone +12, la presión seca al rival −18). Triángulo de posiciones en `BEATS` (DEL>MED>DEF>DEL, ±10/−8 puntos; el POR resiste disparos). Fichaje solo tras ganar: `captureChance()` crece con el margen de goleada; si cambias las fórmulas, la UI se actualiza sola.
